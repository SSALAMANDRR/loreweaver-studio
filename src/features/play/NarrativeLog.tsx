  ...
  return <article className="log-entry combat-result" aria-label={t("combat.result")}>
    ...
    <div className="entry-body">
      ...
      {result.hits.map((hit, index) => {
        ...
        return <p key={index}>{t("combat.hitNumber", { number: index + 1 })}: {location} · {t("combat.damage")}: {mitigated ? `${String(hit.raw_damage)} − ${String(hit.armour_after_penetration)} − ${String(hit.tb_reduction)} = ` : ""}{String(hit.final_damage)}{hit.degrees_substituted === true ? ` (${t("combat.degreesDie")})` : ""}</p>
      })}
